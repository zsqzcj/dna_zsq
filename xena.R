...
rownames(counts1) <- counts1[,1] #Alt -, <- ,第一列变行名
x <- counts1[,1:3]#这是个例子
counts1 = counts1[,-1]#-1就是不要1
#substr函数
substr("wanglihong",1,4)#sub函数把这个字符1-4位提取出来
#table函数
table(substr(colnames(counts1),14,16))#table函数将sub里的14，16位提取出来，分组计算
#c("01A","11A")
#%in%符号用于判断是否属于
counts1 <- counts1[,substr(colnames(counts1),14,16)%in% c("01A","11A")]#01A是癌症组织，#11A是正常
...
library(stringr)#http://127.0.0.1:38569/graphics/plot_zoom_png?width=988&height=900
#设置参考水平
...
dev.off()
#使用R包转换id
index = gset[[1]]@annotation###获得基因平台
...
##读取整理好的表达数据exp##
exp <- read.table("exp1.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
####差异分析####
library(limma)
...
bc <- read.table(file = "deg_all.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
logFC=0.2
P.Value = 0.05
k1 = (bc$P.Value < P.Value)&(bc$logFC < -logFC)#下调的
k2 = (bc$P.Value < P.Value)&(bc$logFC > logFC)#上调的
bc$change = ifelse(k1,"down",ifelse(k2,"up","stable"))
table(bc$change)
#bc <- bc %>% filter(change!="stable")
#table(bc$change)

bc$rownames = row.names(bc)
bc

bc$P.Value = -log10(bc$P.Value)
bc <- as.data.frame(bc)
ggplot(bc, aes(logFC,P.Value))+
  geom_point(size = 2,aes(color = change))+
  scale_x_continuous(limits = c(-0.5, 0.5))+
  geom_label_repel(data = bc,aes(logFC, P.Value, label = rownames,size = 2))
...
logFC=0.2
P.Value = 0.05
k1 = (deg$P.Value < P.Value)&(deg$logFC < -logFC)#下调的
k2 = (deg$P.Value < P.Value)&(deg$logFC > logFC)#上调的
deg$change = ifelse(k1,"down",ifelse(k2,"up","stable"))
table(deg$change)
...
ego_res <- ego@result
save(ego,ego_res,file = "Go_fuji.Rdata")
#3. 可视化
##3.1 柱状图
...
#柱状图
barplot(kk, showCategory = 15,color = "pvalue")
#气泡图
